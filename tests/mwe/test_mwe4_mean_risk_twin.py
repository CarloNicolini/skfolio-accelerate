"""MWE 4: MeanRisk VARIANCE twin vs MeanRisk.fit."""
...
def test_variance_twin_matches_mean_risk_fit():
    X = synthetic_returns(60, 6, seed=7)
    for l2 in (0.0, 1e-4, 1e-3, 1e-2):
        ...
        template = extract_problem_template(twin, f"var-{l2}")
        result = PythonClarabelEngine(template).solve(instantiate(template))
        np.testing.assert_allclose(
            result.weights, fitted.weights_, rtol=1e-5, atol=1e-6
        )
