"""Parity tests for massive_cross_val_predict vs skfolio cross_val_predict."""
...
from skfolio_accelerate.predict import path_predict_blocked_reason
...
def test_compact_variance_matches_mean_risk():
    ...
    engine = make_compact_engine(estimator_spec(MeanRisk(l2_coef=1e-4)), n_assets=8, n_observations=None)
...
def test_walkforward_path_matches_skfolio():
    ...
    est = MeanRisk()
    ref = cross_val_predict(est, X, cv=cv)
    ...
    assert report.n_native_solves == cv.get_n_splits(X)
    assert report.n_prior_fits < report.n_native_solves
...
def test_cpcv_population_matches_skfolio():
    ...
    ref = path_sharpes(cross_val_predict(MeanRisk(l2_coef=1e-3), X, cv=cv))
    pred, report = massive_cross_val_predict(
        MeanRisk(l2_coef=1e-3), X, cv=cv, n_jobs=1, return_report=True
    )
    np.testing.assert_allclose(path_sharpes(pred), ref, rtol=1e-3, atol=1e-4)
    assert report.n_native_solves == cv.get_n_splits()
...
def test_smoke_mrc_matches_skfolio():
    ...
    ref = path_sharpes(cross_val_predict(MeanRisk(), X, cv=cv, n_jobs=1))
    pred, report = massive_cross_val_predict(
        MeanRisk(), X, cv=cv, n_jobs=1, return_report=True
    )
...
def test_kfold_still_works():
    ...
    ref = cross_val_predict(MeanRisk(), X, cv=cv)
    pred = massive_cross_val_predict(MeanRisk(), X, cv=cv, n_jobs=1)
    np.testing.assert_allclose(pred.sharpe_ratio, ref.sharpe_ratio, rtol=1e-3, atol=1e-4)


def test_default_empirical_and_blocked_mip():
    assert is_default_empirical(MeanRisk())
    assert path_predict_blocked_reason(MeanRisk(cardinality=3)) is not None
