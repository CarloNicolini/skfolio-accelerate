...
def test_persist_modes_return_feasible_weights(mode):
    ...
    if mode not in {"cold", "mode_a"}:
        assert engine.n_warm_starts >= 1


def test_mode_e_reuses_numerical_factor_only_when_p_and_a_identical():
    """Mode E must verify P/A identity; changing covariance forbids factor reuse."""
    X = synthetic_returns(60, 4, seed=109)
    spec = _spec(MeanRisk(l2_coef=1e-5))
    engine = make_cosmo_engine(
        spec, n_assets=4, n_observations=None, persist_mode="mode_e"
    )
    from skfolio_accelerate.moments import empirical_from_window as moments_of

    x_arr = np.asarray(X, dtype=np.float64)
    first = moments_of(x_arr[:40], keep_returns=False)
    second = moments_of(x_arr[4:44], keep_returns=False)
    engine.solve(first, warm=False)
    engine.solve(second, warm=True)
    assert engine.last_trace is not None
    assert engine.last_trace.p_identical is False
    assert engine.last_trace.numerical_factor_reused is False
    # Same moments again: P and A identical → numerical factor may be kept.
    engine.solve(second, warm=True)
    assert engine.last_trace.p_identical is True
    assert engine.last_trace.a_identical is True
    assert engine.last_trace.numerical_factor_reused is True
    assert engine._workspace.n_numerical_factor_reuses >= 1
