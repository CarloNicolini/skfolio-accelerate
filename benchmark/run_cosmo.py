...
def profile_fold_breakdown(
    ...
    *,
    ...
    proj_s = sum(t.proj_time for t in traces)
    iters = [t.iterations for t in traces]
    steps = [t.weight_step for t in traces if t.weight_step == t.weight_step]
    dual_steps = [t.dual_step for t in traces if t.dual_step == t.dual_step]
    return {
        ...
        "mean_weight_step": float(np.mean(steps)) if steps else float("nan"),
        "mean_dual_step": float(np.mean(dual_steps)) if dual_steps else float("nan"),
        "n_numerical_factor_reuses": float(engine._workspace.n_numerical_factor_reuses),
        "n_rebuilds": float(engine._workspace.n_rebuilds),
