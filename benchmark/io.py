...
BASELINE_COMMAND = "python benchmark/run_benchmark.py --baseline --workers 1"


def write_baseline_pointer(
    results_root: Path,
    run_dir: Path,
    *,
    environment: dict[str, Any],
    command: str = BASELINE_COMMAND,
) -> Path:
    """Record which historical run is the current official baseline.

    The pointer file is overwritten on each ``--baseline`` run; the run
    directory itself is never overwritten.
    """
    repo_relative = f"benchmark/results/{run_dir.name}"
    payload = {
        "label": "baseline",
        "protocol": "default CONFIG (not --quick, not --full)",
        "command": command,
        "run_directory": run_dir.name,
        "results_csv": f"{repo_relative}/results.csv",
        "results_json": f"{repo_relative}/results.json",
        "summary_md": f"{repo_relative}/summary.md",
        "environment_json": f"{repo_relative}/environment.json",
        "git_sha": environment.get("git_sha"),
        "timestamp": environment.get("timestamp"),
        "skfolio": (environment.get("packages") or {}).get("skfolio"),
        "speedup_definition": "native_time / accelerated_time",
        "reported_time": "median of raw repetitions (warm-ups excluded)",
    }
    path = results_root / BASELINE_POINTER_NAME
    write_json(path, payload)
    (run_dir / "BASELINE").write_text(
        "Official baseline run. See ../baseline.json.\n", encoding="utf-8"
    )
    return path


def parse_baseline_pointer(path: Path) -> dict[str, Any]:
    ...
    if missing:
        raise ValueError(f"baseline pointer missing fields: {missing}")
    if payload.get("label") != "baseline":
        raise ValueError("baseline pointer label must be 'baseline'")
    return payload
