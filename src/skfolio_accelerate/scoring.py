"""Assemble MultiPeriodPortfolio / Population from fold weights."""
...
def ranking_precision_at_k(
    ...
    threshold = ref[np.argsort(-ref, kind="stable")[k - 1]] - score_tolerance
...
def path_sharpes_from_weights(
    ...
    for fold in cv_plan.folds:
        ...
        segments = (
            zip(fold.test_segments, fold.path_ids, strict=False)
            if cv_plan.combinatorial
            else ((fold.test_idx, fold.path_id),)
        )
        for rows, path_id in segments:
            selector = _contiguous_row_slice(rows) or rows
            returns = (
                matrix[selector] @ weights
                if fold.asset_idx is None
                else matrix[selector][:, fold.asset_idx] @ weights
            )
    ...
    for path_id, parts in enumerate(path_returns):
        ...
        sharpes[path_id] = np.mean(returns) / vol if vol else np.nan
    return sharpes
...
def _test_observations(X, idx, cols, *, x_np):
    ...
    row_sel = selector if selector is not None else rows
...
def make_segment_portfolio(
    ...
    return Portfolio(
        X=_test_observations(X, idx, cols, x_np=matrix),
    ...
    *,
    ...
    if cv_plan.combinatorial or cv_plan.multi_path:
        ...
        for fold in cv_plan.folds:
            ...
            pairs = (
                zip(fold.test_segments, fold.path_ids, strict=True)
                if cv_plan.combinatorial
                else ((fold.test_idx, fold.path_id),)
            )
            for seg, path_id in pairs:
                if len(seg):
                    path_lists[path_id].append(segment(w, seg, fold.asset_idx))
