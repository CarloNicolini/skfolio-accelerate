...
# Canonical experimental modes (task naming) and legacy aliases.
# A cold | B warm_primal | C warm_primal_dual
# D persist_structure | E persist_numerical
PersistMode = Literal[
    "cold",
    "warm_x",
    "warm_xy",
...
_MODE_ALIASES: dict[str, str] = {
    ...
    "mode_b": "warm_x",
    "warm_xy": "warm_xy",
    "warm_primal_dual": "warm_xy",
    "mode_c": "warm_xy",
    "persist_factor": "persist_factor",
    "persist_structure": "persist_factor",
    "mode_d": "persist_factor",
    "persist_full": "persist_full",
    "persist_numerical": "persist_full",
    "mode_e": "persist_full",
}
...
def normalize_persist_mode(mode: str) -> str:
    """Map task / alias names onto the five canonical persist modes."""
    key = str(mode).lower()
    if key not in _MODE_ALIASES:
        raise ValueError(
            f"Unknown persist_mode {mode!r}; expected one of "
            f"{sorted(set(_MODE_ALIASES))}"
        )
    return _MODE_ALIASES[key]


def _csc_equal(left: sp.csc_matrix, right: sp.csc_matrix, *, atol: float = 0.0) -> bool:
    """Exact CSC identity (shape, pattern, and numeric values)."""
    ...
    return bool(np.allclose(left.data, right.data, rtol=0.0, atol=atol))
...
class CosmoFoldTrace:
    ...
    restarted: bool
    weight_step: float
    dual_step: float = float("nan")
    p_identical: bool = False
    a_identical: bool = False
    numerical_factor_reused: bool = False
    persist_mode: str = "cold"
...
class PersistentCosmo:
    ...
    Persist modes (aliases accepted; canonical name in parentheses):

    * Mode A ``cold`` — new solver every fold, default ADMM state.
    * Mode B ``warm_x`` / ``warm_primal`` — new solver, warm-start ``x``.
    * Mode C ``warm_xy`` / ``warm_primal_dual`` — new solver, warm-start
      ``x`` and dual ``y`` (COSMO.rs has no public ``s`` warm-start; slack is
      recovered inside ADMM from ``A x + s = b``).
    * Mode D ``persist_factor`` / ``persist_structure`` — keep the workspace;
      update ``P``/``A``/``q``/``b``; reset ADMM iterates. Same-sparsity
      ``update_p`` numerically refactors (QDLDL symbolic analysis reused by
      ``rebuild``). ``update_a`` always drops the KKT object in COSMO.rs.
    * Mode E ``persist_full`` / ``persist_numerical`` — keep ADMM state when
      warm, but **reuse the numerical factorisation only when ``P`` and ``A``
      are verified numerically identical** to the previous fold. If either
      matrix changed, Mode E still updates data (like Mode D) and does not
      claim numerical-factor reuse.
    """
    ...
    def __init__(
        ...
        *,
        ...
        self.persist_mode = normalize_persist_mode(persist_mode)
        ...
        self.n_restarts = 0
        self.n_rebuilds = 0
        self.n_numerical_factor_reuses = 0
    ...
    def solve(
        ...
        *,
        ...
        p_identical = self._P is not None and _csc_equal(self._P, P)
        a_identical = self._A is not None and _csc_equal(self._A, A)
        # Mode E: numerical KKT reuse is valid only when P and A are identical.
        numerical_ok = p_identical and a_identical
        if mode == "persist_full":
            # Do not call update_p/update_a when matrices are identical; that
            # is the only case where the existing numerical factor stays valid.
            p_changed = p_changed and not p_identical
            a_changed = a_changed and not a_identical
        ...
        if self.solver is None or reconstruct:
            ...
            if b_changed:
                ...
                updated_b = True
            if mode == "persist_full" and numerical_ok and not (updated_p or updated_a):
                numerical_factor_reused = True
                self.n_numerical_factor_reuses += 1
            if not warm or mode == "persist_factor":
                self.solver.reset("factor" if mode == "persist_factor" else "cold")
            elif mode == "persist_full" and warm and not numerical_ok:
                # P or A changed: keep ADMM warm-start but do not pretend the
                # previous numerical factor is still valid (update_* already
                # dropped or rebuilt it).
                pass
            if warm:
                self.n_warm_starts += 1
        ...
        s = np.asarray(getattr(solution, "s", []), dtype=np.float64)
        step = (
            float("nan")
            if self._x is None
        ...
        dual_step = (
            float("nan")
            if self._y is None
            else float(np.linalg.norm(y[: self._y.size] - self._y[: y.size]))
        )
        self._x = x.copy()
        self._y = y.copy()
        self._s = s.copy() if s.size else None
        self._P = P.copy()
        self._A = A.copy()
        trace = CosmoFoldTrace(
            ...
            numerical_factor_reused=numerical_factor_reused,
            persist_mode=mode,
        )
