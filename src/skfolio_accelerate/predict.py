...
class AccelerationReport:
    ...
    def __str__(self) -> str:
        extra = (
            f"\nBaseline {self.baseline_s:.4f}s  speedup {self.speedup:.2f}×"
    ...
    *,
    ...
    if backend == "sklearn" or (
        ...
        report = AccelerationReport(
            backend="sklearn",
            reason=_choice_reason("sklearn", capabilities),
            fallback_reason="backend=sklearn"
            if backend == "sklearn"
            else (
                capabilities.assemble_reason
                or capabilities.sequential_reason
    ...
    if capabilities.can_compact and type(estimator) in _CLOSED_FORM:
        merged = merge_batch_results(
            [
                closed_form_weights(
                    x_arr, batch, estimator, fold_blocks=cv_plan.fold_blocks
                )
                for batch in cv_plan.path_batches()
            ]
        )
        pred, eval_s = _assemble(
            X, cv_plan, merged, name, portfolio_params=portfolio_params
        )
        report = _report(
            "closed-form",
            merged,
