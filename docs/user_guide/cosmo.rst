...
Default persist mode is ``persist_full`` (Mode E / numerical reuse when
``P`` and ``A`` are identical; otherwise update + warm ADMM) for variance
and ``persist_factor`` (Mode D / structural reuse) for scenario risks.
Modes A–C reconstruct the solver each fold (cold / warm ``x`` / warm
``x,y``). Full ADMM-state reuse *increased* iteration count on class-B
problems in the measured panel. Mode E **verifies** ``P``/``A`` identity
before claiming numerical-factor reuse; covariance-changing variance folds
therefore do not keep the previous LDLᵀ factor.
